...
import { Database, Camera, HardDrive, AlertTriangle, Cpu, MemoryStick } from "lucide-react";
...
export const proxmoxBackupServerDefinition: ServiceDefinition<ProxmoxBackupServerData> = {
  ...
  async fetchData(config) {
    ...
    const [dsRes, statusRes, tasksRes] = await Promise.all([
      ...
      fetchWithTimeout(`${baseUrl}/api2/json/nodes/localhost/tasks?errors-only=1&limit=100`, {
        headers,
      }),
    ]);
