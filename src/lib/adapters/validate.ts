...
export function validateResponse<T = Record<string, unknown>>(
  ...
  if (value == null) {
    ...
    throw new Error(
      `[${adapter}] API returned null/undefined instead of a JSON object`
    )
  }
  ...
  if (!isPlainObject(value)) {
    if (optional) return {} as T
    throw new Error(
      `[${adapter}] API returned ${typeLabel(value)} instead of a JSON object`
    )
  }
  ...
  if (missing.length > 0) {
    throw new Error(
      `[${adapter}] API response missing required field(s): ${missing.join(", ")}`
    )
  }
  ...
  for (const spec of nested) {
    ...
    if (fieldValue === undefined) {
      ...
      if (spec.type) {
        throw new Error(
          `[${adapter}] API response missing nested field "${spec.path}"`
        )
      }
      continue
    }
 ...
 */
export function validateArrayResponse<T = unknown>(
  value: unknown,
  options: ValidateOptions
): T[] {
  const { adapter, optional = false } = options
  ...
  if (value == null) {
    if (optional) return []
    throw new Error(
      `[${adapter}] API returned null/undefined instead of a JSON array`
    )
  }
