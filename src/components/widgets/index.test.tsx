...
describe("StatCard", () => {
  ...
  it("renders icon when provided", () => {
    ...
    renderWithTooltip(
      <StatCard id="test-2" value="Test" label="With Icon" icon={TestIcon} displayMode="icon" />,
    );
  ...
  it("places icon below value in DOM order", () => {
    ...
    const { container } = renderWithTooltip(
      <StatCard id="test-3" value="42" label="Test" icon={TestIcon} displayMode="icon" />,
    );
  ...
  it("applies custom valueClassName", () => {
    const { container } = render(
      <StatCard id="test-4" value="100" label="Custom" valueClassName="text-destructive" />,
    );
...
describe("ActiveStreamItem", () => {
  ...
  it("formats long duration with hours", () => {
    renderWithTooltip(
      <ActiveStreamItem title="Long Movie" user="user" progress={3661} duration={7200} />,
    );
  ...
  it("handles zero duration gracefully", () => {
    renderWithTooltip(
      <ActiveStreamItem title="Zero Duration" user="user" progress={0} duration={0} />,
    );
...
describe("ActiveStreamList", () => {
  ...
  it("sorts streams alphabetically by title", () => {
    ...
    const titles = screen
      .getAllByText(/.+/)
      .filter((el) => ["Zebra", "Alpha", "Middle"].includes(el.textContent || ""));
    // Check order in DOM
...
describe("DownloadItem", () => {
  ...
  it("shows activity without timeLeft", () => {
    renderWithTooltip(<DownloadItem title="No Time" progress={75} activity="seeding" />);
    expect(screen.getByText("seeding")).toBeInTheDocument();
  });

  it("clamps progress to 0-100 range visually", () => {
    const { container: c1 } = renderWithTooltip(<DownloadItem title="Over" progress={150} />);
    const bar1 = c1.querySelector('[style*="width: 100%"]');
    expect(bar1).toBeInTheDocument();

    const { container: c2 } = renderWithTooltip(<DownloadItem title="Zero" progress={0} />);
