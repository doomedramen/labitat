...
test.describe("Edit Mode", () => {
  ...
  test("enters edit mode when clicking Edit", async ({ page }) => {
    await page.getByRole("button", { name: "Edit" }).click()
    await expect(
      page.getByText("Drag to reorder. Click items to edit.")
    ).toBeVisible()
  ...
  test("deletes a group with confirmation", async ({ page }) => {
    ...
    await expect(
      page.locator("h2", { hasText: "Infrastructure" })
    ).not.toBeVisible()
  ...
  test("deletes an item with confirmation", async ({ page }) => {
    ...
    await expect(
      page.getByTestId("item-card").filter({ hasText: "Proxmox" })
    ).toHaveCount(0)
    await expect(
      page.getByTestId("item-card").filter({ hasText: "Grafana" })
    ).toBeVisible()
  })
})
