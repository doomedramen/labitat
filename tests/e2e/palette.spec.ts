...
test.describe("Palette Switching", () => {
  ...
  test("shows all palette options", async ({ page }) => {
    ...
    for (const palette of PALETTES) {
      await expect(
        page.getByRole("menuitemradio", { name: palette, exact: true })
      ).toBeVisible()
    }
  })

  test("switches palette and applies data-palette attribute", async ({
    page,
  }) => {
    await page.getByRole("button", { name: "Theme settings" }).click()
  ...
  test("can switch between multiple palettes", async ({ page }) => {
    ...
    await page.getByRole("button", { name: "Theme settings" }).click()
    await page
      .getByRole("menuitemradio", { name: "Gruvbox", exact: true })
      .click()
    ...
    await page.getByRole("button", { name: "Theme settings" }).click()
    await page
      .getByRole("menuitemradio", { name: "Catppuccin", exact: true })
      .click()
